...
class MorphObfuscator {
    ...
    /// 使用外部传入的 fnInitor 参数初始化（Extension 进程复用 App 进程握手参数）
    init(key: Int, layer: Int, paddingLength: Int, substitutionTable: [UInt8], randomValue: UInt8) {
        self.key = key
        self.layer = layer
        self.paddingLength = min(max(paddingLength, 1), 8)
        self.functionRegistry = FunctionRegistry(layer: layer, substitutionTable: substitutionTable, randomValue: randomValue)
        self.totalCombinations = functionRegistry.getTotalCombinations()
        ...
        NSLog("🎭 MorphObfuscator(ext): key=\(key) layer=\(layer) pad=\(self.paddingLength) combos=\(totalCombinations)")
    }
